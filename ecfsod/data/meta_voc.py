...
__all__ = ["register_meta_nwpu", "register_meta_dior"]

def load_filtered_nwpu_instances(name: str, dirname: str, split: str, classnames: str):
    is_shots = "shot" in name
    if is_shots:
        fileids = {}
        split_dir = os.path.join("datasets", "nwpusplit")
    ...
    if is_shots:
        for cls, fileids_ in fileids.items():
            ...
            for fileid in fileids_:
                dirname = os.path.join("datasets", "nwpu")
...
def load_filtered_dior_instances(name: str, dirname: str, split: str, classnames: str):
    is_shots = "shot" in name
    if is_shots:
        fileids = {}
        split_dir = os.path.join("datasets", "diorsplit")
        shot = name.split("_")[-2].split("shot")[0]
        seed = int(name.split("_seed")[-1])
        split_dir = os.path.join(split_dir, "seed{}".format(seed))
        for cls in classnames:
    ...
    if is_shots:
        for cls, fileids_ in fileids.items():
            dicts_ = []
            for fileid in fileids_:
                dirname = os.path.join("datasets", "dior")
                anno_file = os.path.join(
                    dirname, "Annotations", fileid + ".xml"
                )
                jpeg_file = os.path.join(
...
def register_meta_nwpu(name, metadata, dirname, split, keepclasses, sid):
    ...
    DatasetCatalog.register(
        name,
        lambda: load_filtered_nwpu_instances(
...
def register_meta_dior(name, metadata, dirname, split, keepclasses, sid):
    if keepclasses.startswith("base_novel"):
        thing_classes = metadata["thing_classes"][sid]
    elif keepclasses.startswith("base"):
        thing_classes = metadata["base_classes"][sid]
    ...
    DatasetCatalog.register(
        name,
        lambda: load_filtered_dior_instances(
            name, dirname, split, thing_classes
        ),
    )
