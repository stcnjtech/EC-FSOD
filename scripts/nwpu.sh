SPLIT_ID=1 # <-- change split_id 1,2
SAVE_DIR=checkpoints/nwpu_dataset
...
# ------------------------------- Pre-train ----------------------------------
python3 main.py --num-gpus 1 --config-file configs/nwpu/ecfsod_det_r101_base${SPLIT_ID}.yaml \
    --opts MODEL.WEIGHTS ${IMAGENET_PRETRAIN}                                                \
           OUTPUT_DIR ${SAVE_DIR}/${SPLIT_NAME}/pretrain/

# ----------------------------- Model Preparation ---------------------------------
python3 tools/model_surgery.py --dataset nwpu --method randinit                              \
...
do
    ...
    do
        python3 tools/create_config.py --dataset nwpu --config_root configs/nwpu             \
            --shot ${shot} --seed ${seed} --setting 'gfsod' --split ${SPLIT_ID}

        CONFIG_PATH=configs/nwpu/ecfsod_gfsod_r101_novel${SPLIT_ID}_${shot}shot_seed${seed}.yaml
